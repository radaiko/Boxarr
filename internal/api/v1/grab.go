...
func (h *Handler) grabRelease(ctx context.Context, ref grabRef, mediaType string, mediaRef int64) (*job.Job, bool, error) {
	...
	if ref.Protocol == "torrent" {
		...
		case ref.DownloadURL != "":
			...
			if b, err := prowlarr.FetchArtifact(ctx, ref.DownloadURL, h.deps.Settings.ProwlarrURL(), h.deps.Settings.ProwlarrAPIKey()); err == nil {
				j.TorrentFile = b
			} else if hash != "" {
	...
	b, err := prowlarr.FetchArtifact(ctx, ref.DownloadURL, h.deps.Settings.ProwlarrURL(), h.deps.Settings.ProwlarrAPIKey())
	if err != nil {
